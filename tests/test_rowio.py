...
class OdsRowsTest(_BaseRowsTest):
    ...
    def test_fails_on_ods_from_csv(self):
        ...
        try:
            ...
            self.assertTrue("cannot uncompress ODS spreadsheet:" in error_message, "error_message=%r" % error_message)
    ...
    def test_fails_on_ods_without_content_xml(self):
        ...
        try:
            ...
            error_message = "%s" % error
            self.assertTrue("cannot extract content.xml" in error_message, "error_message=%r" % error_message)
    ...
    def test_fails_on_ods_without_broken_content_xml(self):
        ...
        try:
            ...
            error_message = "%s" % error
            self.assertTrue("cannot parse content.xml" in error_message, "error_message=%r" % error_message)
    ...
    def test_fails_on_non_existent_ods_sheet(self):
        ...
        try:
            ...
            error_message = "%s" % error
            self.assertTrue("ODS must contain at least" in error_message, "error_message=%r" % error_message)
...
class DelimitedRowsTest(_BaseRowsTest):
    ...
    def test_fails_on_delimited_with_unterminated_quote(self):
        ...
        try:
            ...
            error_message = "%s" % error
            self.assertTrue("cannot parse delimited file" in error_message, "error_message=%r" % error_message)
...
class FixedRowsTest(_BaseRowsTest):
    ...
    def test_can_read_empty_fixed_rows(self):
        ...
        with io.StringIO("") as data_io:
            rows = list(rowio.fixed_rows(data_io, data_format.encoding, (("dummy", 1),), data_format.line_delimiter))
    ...
    def test_can_read_fixed_rows_with_missing_terminating_line_delimiter(self):
        ...
        with io.StringIO("hugo172\nsepp163") as data_io:
            rows = list(
                rowio.fixed_rows(data_io, data_format.encoding, field_names_and_lengths, data_format.line_delimiter)
            )
        self._assert_rows_contain_data(rows)
...
class DelimitedRowWriterTest(unittest.TestCase):
    ...
    def test_can_write_delimited_data_to_path(self):
        ...
        with io.open(
            delimited_path, "w", newline="", encoding=delimited_data_format.encoding
        ) as delimited_target_stream:
...
class XlsxRowWriterTest(unittest.TestCase):
    def test_can_write_xlsx(self):
        ...
        for row_index, row_read in enumerate(rowio.excel_rows(xlsx_path)):
            string_row_written = [str(item) for item in rows_to_write[row_index]]
