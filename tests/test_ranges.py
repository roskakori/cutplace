...
from cutplace import errors, ranges
...
class RangeTest(unittest.TestCase):
    ...
    def test_can_create_range_from_length(self):
        ...
        self.assertEqual(
            ranges.create_range_from_length(ranges.Range("3...8")).items, [(-9999999, -10), (100, 99999999)]
        )
        ...
        self.assertEqual(
            ranges.create_range_from_length(ranges.Range("3...4, 10...")).items,
            [(-999, -10), (100, 9999), (None, -100000000), (1000000000, None)],
        )
...
class DecimalRangeTest(unittest.TestCase):
    def test_can_handle_proper_decimal_ranges(self):
        ...
        self.assertEqual(
            ranges.DecimalRange("1.1" + "\u2026" + "2.1").items, [(decimal.Decimal("1.1"), decimal.Decimal("2.1"))]
        )
    ...
    def test_fails_on_value_out_of_range(self):
        ...
        dev_test.assert_raises_and_fnmatches(
            ...
            "x is Decimal('-1.3') but must be within range: '-1.2...1.5'",
            lower_and_upper_range.validate,
            "x",
            "-1.3",
        )
    ...
    def _test_fails_with_interface_error(self, description, anticipated_error_message_pattern):
        dev_test.assert_raises_and_fnmatches(
            self, errors.InterfaceError, anticipated_error_message_pattern, ranges.DecimalRange, description
        )
    ...
    def test_fails_on_parse_hex_range(self):
        dev_test.assert_raises_and_fnmatches(
            ...
            "number must be an decimal or integer but is: '0x7f'",
            ranges.DecimalRange,
            "0x7f",
        )
    ...
    def test_can_parse_multiple_ranges(self):
        self.assertEqual(
            ranges.DecimalRange("1.1, 3").items, [(decimal.Decimal("1.1"), decimal.Decimal("1.1")), (3, 3)]
        )
